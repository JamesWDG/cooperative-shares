                ...
                <div class="col-xxl-9 col-xl-8">
                    ...
                    <form id="profile-form">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <div>:</div>
                            <input class="input-area" type="text" name="phone_number" value="{{ $user->phone_number }}" class="form-control" placeholder="Phone Number">
                        </div>
                ...
                <div class="col-xxl-3 col-xl-4">
                    <div class="profile-card shadow-sm text-center">
                        ...
                        <img id="profileImage"
                            ...
                            src="{{ $user->profile_image ? asset($user->profile_image) : asset('assets/dummy_avatar/avatar.jpg') }}"
                            alt="Profile"
                        ...
                        <!-- Profile Image --><br/>
                        <img id="profileLogo"
                            onclick="document.getElementById('profileLogoInput').click()"
                            title="Click Here To Upload Profile Logo"
                            src="{{ $user->profile_logo ? asset($user->profile_logo) : asset('assets/dummy_avatar/avatar.jpg') }}"
                            alt="Profile"
                            class="img-fluid rounded mb-2"
                            style="max-height: 180px; object-fit: cover;">
                        ...
                        <!-- Upload & Delete Buttons -->
                        <h6 class="change-pro-hd">Profile Logo</h6>
                        <div class="mb-2 d-flex justify-content-center profile-pic-form">
                            <form id="profile-logo-form">
                                @csrf
                                <input type="file" id="profileLogoInput" name="profile_logo" style="display: none;">
                                <button class="btn-sm upload-btn-primary">SAVE LOGO</button>
                            </form>
                            <button class="btn-sm btn-danger" {{ $user->profile_logo ? '' : 'disabled' }} id="deleteProfileLogo">DELETE</button>
                        </div>
                        <!-- FULL WIDTH IMAGE GALLERY BUTTON -->
                        <div class="mt-3">
                            <a href="{{route('vendor.gallery')}}" class="btn btn-primary w-100 gallery-btn">
                                <i class="fa-solid fa-image"></i> &nbsp; Click to view Image Gallery
                            </a>
                        </div>
                    </div>
                </div>
...
@push('scripts')
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#profile-logo-form').on('submit', function (e) {
                e.preventDefault();

                let imageInput = $('#profileLogoInput')[0];
                let allowedTypes = ["image/jpeg", "image/png", "image/jpg", "image/gif"];
                if(!imageInput.files.length)
                {
                    Swal.fire({
                ...
                let formData = new FormData(this);
                $.ajax({
                    url: "{{ route('vendor.profileLogo.store') }}",
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#deleteProfileLogo').on('click', function(){

                let formData = new FormData();
                formData.append('_token', "{{ csrf_token() }}");
                formData.append('_method', 'delete');
                $.ajax({
                    url: "{{ route('vendor.profileLogo.destory') }}",
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
        ...
        $(document).ready(function () {
            $('#imageInput').on('change', function (e) {
                ...
                if (!allowedTypes.includes(file.type)) {
                    Swal.fire({
                        title: "Error!",
                        text: "Please upload an image (jpeg, png, jpg, gif).",
                        icon: 'error',
                        confirmButtonColor: '#295568',
                        confirmButtonText: 'OK'
                    });
            ...
            $('#profileLogoInput').on('change', function (e) {
                const file = this.files[0];
                if (!file) return;
                ...
                const reader = new FileReader();
                reader.onload = function (event) {
                    $('#profileLogo').attr('src', event.target.result);
                };
                reader.readAsDataURL(file);
            });
        });
